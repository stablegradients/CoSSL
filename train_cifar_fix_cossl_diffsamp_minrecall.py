...
parser.add_argument('--wandb-project', default="CoSSL",
...
def validate_teacher(valloader, model, head, criterion, use_cuda,\
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, _) in enumerate(valloader):
            ...
            outputs_.append(torch.argmax(outputs, dim=1).cpu().detach().numpy())
            targets_.append(targets.cpu().detach().numpy())
    ...
    if dataset_name == "cifar10":
        ...
        classes = torchvision.datasets.CIFAR100('./BS/databases00/cifar-100', train=True, download=True).classes
    metrics, CM = get_metrics(logits_, labels_, classes)
    wandb.log(metrics)
